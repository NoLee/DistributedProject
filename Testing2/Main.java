...
public class Main {
	...
	{   
		...
		ProcessBuilder pb = new ProcessBuilder("java.exe","-cp","bin","Testing2.Node",""+network.leaderSocket,"1",""+network.socketNum,""+network.leaderSocket);
	...
	public void join(int id) throws NoSuchAlgorithmException, IOException
	{
		...
		ProcessBuilder pb = new ProcessBuilder("java.exe","-cp","bin","Testing2.Node", ""+nodeSocket ,""+id,""+this.socketNum,""+this.leaderSocket);
		pb.inheritIO();
        Process p = pb.start();
